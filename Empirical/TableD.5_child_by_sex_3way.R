  ...
  for (race in race_set){
    ...
    for (sex in sex_set) {
      ...
      for (year in year_set) {
        ...
        data1<-data_now[data_now$SEX==sex&data_now$RACE==race&data_now$child==child_set[child,1],]
        data2<-data_now[data_now$SEX==sex&data_now$RACE==race&data_now$child==child_set[child,2],]
        ...
        #lines(z,FFFy2,col='red')
        
        
        
        #N2<-length(unique(data1$PWSTATE2));M2<-length(unique(data2$PWSTATE2))
...
meta_data <- expand.grid(
  ...
  arrange(desc(Direction), Race, Sex, Year) 
...
print(table_output)
 ...
 write.csv(table_output, "2_3way_Empirical_Child_by_sex.csv", row.names = FALSE)
