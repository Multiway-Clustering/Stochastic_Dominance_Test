...
edu_set<-c('1_Less_than_HS','2_HS_Graduate','3_Some_College','4_Bachelors','5_Advanced_Degree')
race_set<-rbind(c(1,2),c(2,1))
alpha<-0.05
Test_stat_M<-NULL
pvalues_M<-NULL

for (race in 1:2) {
  cat(sprintf("\n--- Running for RACE = %d vs. RACE = %d ---\n", race_set[race,1],race_set[race,2]))
  for (edu in edu_set) {
    cat(sprintf("  --> Running for EDU = %s ---\n", edu))
    for (year in year_set) {
      cat(sprintf("    ->> Running  for YEAR = %d\n", year))
      data_now<-data[data$YEAR==year,]
      
      data1<-data_now[data_now$RACE==race_set[race,1]&data_now$edu_group==edu,]
      data2<-data_now[data_now$RACE==race_set[race,2]&data_now$edu_group==edu,]
      ...
      plot(z,Fy1,type='l',ylab="ECDF",xlab='Income ($10,000)',main=c(year,edu))
      lines(z,Fy2,lty=4,col='blue')
      legend("bottomright",legend = c('White','Black'),lty=c(1,4),col=c('black','blue'))
...
meta_data <- expand.grid(
  Year      = year_set,
  Edu_Raw   = edu_set,
  Direction = c("White_vs_Black", "Black_vs_White")
)
...
final_table <- combined_df %>%
  mutate(
    Edu_Lab = str_remove(Edu_Raw, "^\\d_"),
    Metric  = factor(Metric, levels = c("Test Stat", "p values"))
  ) %>%
  arrange(Edu_Raw, Year, Metric) %>%
  mutate(across(where(is.numeric) & !Year, ~ sprintf("%.3f", .)))

table_output <- final_table %>%
  group_by(Edu_Lab) %>%
  mutate(Edu_Display = ifelse(row_number() == 1, as.character(Edu_Lab), "")) %>%
  group_by(Edu_Lab, Year) %>%
  ...
  select(
    ...
    Year = Year_Display, 
    ` ` = Metric,
    starts_with("White_vs_Black"),
    starts_with("Black_vs_White")
  )


print(table_output)
write.csv(table_output, "4_3way_Empirical_Results_Ethnic_Comparison_By_Education.csv", row.names = FALSE)
