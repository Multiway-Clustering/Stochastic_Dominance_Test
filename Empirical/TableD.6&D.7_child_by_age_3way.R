...
age_set<-c('1_18-24','2_25-34','3_35-49','4_50-65')
child_set<-rbind(c(1,0),c(0,1))
alpha<-0.05
Test_stat_M<-NULL
pvalues_M<-NULL

for (child in 1:2) {
  cat(sprintf("\n--- Running for Child = %d vs. Child = %d ---\n", child_set[child,1],child_set[child,2]))
  for (race in race_set){
    cat(sprintf("  --> Running for RACE = %d ---\n", race))
    for (age in age_set) {
      cat(sprintf("    ->> Running for AGE = %s ---\n", age))
      for (year in year_set) {
        ...
        data1<-data_now[data_now$SEX==1&data_now$RACE==race&data_now$age_group==age&data_now$child==child_set[child,1],]
        data2<-data_now[data_now$SEX==1&data_now$RACE==race&data_now$age_group==age&data_now$child==child_set[child,2],]
        
        
        N1<-length(unique(data1$IND));M1<-length(unique(data2$IND));
        N2<-length(unique(data1$OCC));M2<-length(unique(data2$OCC))
        N3<-length(unique(data1$PWSTATE2));M3<-length(unique(data2$PWSTATE2))
        Nbar<-min(N1,N2,N3);Mbar<-min(M1,M2,M3)
        ...
        plot(z,Fy1,type='l',ylab="ECDF",xlab='Income ($10,000)',main=c(year,age))
        lines(z,Fy2,lty=4,col='blue')
        legend("bottomright",legend = c('With children','Without children'),lty=c(1,4),col=c('black','blue'))
        ...
        system.time({
          result =  foreach(k = 1:reps, .combine = 'rbind')%dopar%{
            ...
            #####================================WFH=================================#####
            Y1_bs<-bs_sample_empirical_3way('IND','OCC','PWSTATE2','real_Inwage',data1)
            Fy1_bs<-myECDF2(Y1_bs,z)$Fy
            
            Y2_bs<-bs_sample_empirical_3way('IND','OCC','PWSTATE2','real_Inwage',data2)
...
meta_data <- expand.grid(
  ...
  Age_Raw   = c('1_18-24','2_25-34','3_35-49','4_50-65'),
  Race      = race_set, #  c(1, 2)
...
combined_long <- bind_rows(df_stat, df_p) %>%
...
final_table <- combined_long %>%
  mutate(
    ...
    Age_Lab  = str_remove(Age_Raw, "^\\d_"),
    Metric   = factor(Metric, levels = c("Test Stat", "p values"))
  ) %>%
  arrange(Race_Lab, Age_Raw, Year, Metric) %>%
  mutate(across(where(is.numeric) & !c(Year, Race), ~ sprintf("%.3f", .)))

table_output <- final_table %>%
  group_by(Race_Lab) %>%
  mutate(Race_Display = ifelse(row_number() == 1, as.character(Race_Lab), "")) %>%
  group_by(Race_Lab, Age_Lab) %>%
  mutate(Age_Display = ifelse(row_number() == 1, as.character(Age_Lab), "")) %>%
  group_by(Race_Lab, Age_Lab, Year) %>%
...
write.csv(table_output, "3way_Empirical_Father_Age_Child_Table.csv", row.names = FALSE)
