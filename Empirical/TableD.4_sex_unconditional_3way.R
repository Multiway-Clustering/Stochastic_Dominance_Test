...
for (sex in 1:2) {
  ...
  for (race in race_set){
    ...
    for (year in year_set) {
      ...
      z_min <- min(c(y1, y2)); z_max <- max(c(y1, y2))
      z <- seq(z_min, z_max, length.out = 500)
...
print(table_output)
write.csv(table_output, "0_3way_Emprical_Gender_Dominance_By_Race.csv", row.names = FALSE)
