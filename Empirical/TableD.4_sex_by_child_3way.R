...
for (sex in 1:2) {
  ...
  for (race in race_set){
    ...
    for (child in child_set) {
      cat(sprintf("    ->> Running for CHILD = %d ---\n", child))
      for (year in year_set) {
        cat(sprintf("      >>> Running  for YEAR = %d\n", year))
        
        data_now<-data[data$YEAR==year,]
        
        data1<-data_now[data_now$SEX==sex_set[sex,1]&data_now$RACE==race&data_now$child==child,]
        data2<-data_now[data_now$SEX==sex_set[sex,2]&data_now$RACE==race&data_now$child==child,]
...
) %>% arrange(Direction, Race, Child, Year) 
...
df_stat <- as.data.frame(Test_stat_M) %>%
  bind_cols(meta_data) %>%
  mutate(Type = "Test Stat")

df_p <- as.data.frame(pvalues_M) %>%
  bind_cols(meta_data) %>%
  mutate(Type = "p values")
...
final_df <- bind_rows(df_stat, df_p) %>%
  ...
  mutate(
    Race_Label = factor(ifelse(Race == 2, "Black", "White"), levels = c("Black", "White")),
    Child_Label = factor(ifelse(Child == 1, "With Children", "Without Children"), 
                         levels = c("With Children", "Without Children")),
    Type = factor(Type, levels = c("Test Stat", "p values"))
  ) %>%
  arrange(Race_Label, Child_Label, Year, Type)

table_output <- final_df %>%
  mutate(across(starts_with("Male") | starts_with("Female"), 
                ~ ifelse(is.na(.), "", sprintf("%.3f", as.numeric(.))))) %>%
  group_by(Race_Label) %>%
  mutate(Race_Display = ifelse(row_number() == 1, as.character(Race_Label), "")) %>%
  group_by(Race_Label, Child_Label) %>%
  mutate(Child_Display = ifelse(row_number() == 1, as.character(Child_Label), "")) %>%
  group_by(Race_Label, Child_Label, Year) %>%
...
print(table_output)
write.csv(table_output, "1_3way_Empirical_Results_sex_by_child.csv", row.names = FALSE)
