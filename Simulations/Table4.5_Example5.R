...
save_dir <- paste0("Sim_Results_Example5_SizePower_", timestamp)
  ...
  for (rho in rho_set) {
    ...
    for (mu_y in mu_y_set) {
      ...
      result_matrix = foreach(k = 1:reps, .combine = 'rbind') %dopar% {
        ...
        # DGP for Example 5
        data <- data_gen5_6(N1, N2, M1, M2, mu_y = mu_y, sigma_y = sigma_y_fixed, rho = rho)
        X <- pnorm(data$X)
        Y <- pnorm(data$Y)
        
        # Empirical CDF using safe myECDF2
        z <- sort(unique(c(Y, X)))
        Fx <- myECDF2(X, z)$Fy
        Fy <- myECDF2(Y, z)$Fy
        diff_FyFx <- Fy - Fx
      ...
      names(rej_rates) <- paste0(names(rej_rates), sprintf("(rho=%.1f,mu=%.1f)", rho, mu_y))
...
print(all_results_table[, 1:6]) # 
cat("... (more columns saved to CSV)\n")

file_name <- paste0("Table_Example5_SizePower_Combined_", timestamp, ".csv")
