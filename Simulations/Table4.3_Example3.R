...
# --- Parameter Sets for the Simulation Grid ---
mu_y_set <- c(0.2, 0, -0.2)
sigma_y_fixed <- 1 

# --- Define the sample size scenarios from Table 4.3 ---
...
save_dir <- paste0("Sim_Results_Example3_SizePower_", timestamp)
...
for (i in 1:nrow(all_scenarios)) {
  ...
  for (mu_y in mu_y_set) {
    ...
    result_matrix = foreach(k = 1:reps, .combine = 'rbind') %dopar% {
      ...
      # Data Generating Process (DGP) for Example 3
      data <- data_gen3_4(N1, N2, M1, M2, mu_y = mu_y, sigma_y = sigma_y_fixed)
      Y<-data$data_y$Y;X<-data$data_x$X
...
file_name <- paste0("Table4-3_SizePower_Results_", timestamp, ".csv")
