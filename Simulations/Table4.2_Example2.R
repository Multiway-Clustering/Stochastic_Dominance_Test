...
# These parameters correspond to Table 4.2 (Example 2)
...
save_dir <- paste0("Sim_Results_Example2_Power_", timestamp)
...
for (i in 1:nrow(all_scenarios)) {
  ...
  # A vector to store results for the current row
  current_scenario_results_row <- c(N1_M1 = N1, N2_M2 = N2)
  
  start_time <- proc.time()
  
  # Inner loop for parameter 'sigma_y'
  for (sigma_y in sigma_y_set) {
    ...
    result_matrix = foreach(k = 1:reps, .combine = 'rbind') %dopar% {
      ...
      data <- data_gen1_2(N1, N2, M1, M2, mu_y = mu_y_fixed, sigma_y = sigma_y)
      Y <- pnorm(data$Y)
      X <- pnorm(data$X)
      ...
      for (j in 1:B) {
        ...
        Y_bs_WFH <- na.omit(bs_sample(N1, N2, data$Y))
        X_bs_WFH <- na.omit(bs_sample(M1, M2, data$X))
...
file_name <- paste0("Table4-2_Power_Results_", timestamp, ".csv")
